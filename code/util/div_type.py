...
class div_query:
    ...
    def add_cl_pair(self, cl_pair):
        self.cl_pair = cl_pair

    def get_test_alpha_nDCG(self, docs_rank, topn=20):
        '''
        ...
        metrics = []
        p = 0.5
        real_num = min(topn, len(docs_rank))
        best_docs_index = []
...
class CLDataset(Dataset):
    ...
    def __getitem__(self,  idx):
        ...
        intent_doci = self.data[idx][12]
        intent_docj = self.data[idx][13]
        intent_label = torch.tensor(self.data[idx][14]).float()
        ...
        intent_doci.requires_grad = False
        intent_docj.requires_grad = False
        return doc_emb, sub_emb, doc_mask, sub_mask, weight, index_i, index_j, pos_qrel_feat, neg_qrel_feat, \
               pos_subrel_feat, neg_subrel_feat, subrel_mask#, intent_doci, intent_docj, intent_label
